...
export default function FormProducts({ setAgregarProducto, setAuxListProduct, setAgregarPedidos, UserDataJson, setAuxOrder }) {
    const { dataSucursal, setAux } = useFront()
    const [open, setOpen] = useState(false);
    const [productData, setProductData] = useState({});
    const [productsTable, setProductsTable] = useState([]);

    const schema = UserDataJson?.idType !== 2 ?  yup.object().shape({
        ...
        <div className="w-full">
            ...
            >{({ handleSubmit, handleChange, setFieldValue, values, resetForm, errors, touched }) => (
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 md:gap-6 ">
                    ...
                    {/* {UserDataJson?.idType !== 2 && <div className="">
            ...
            {productsTable.length > 0 && <TablaProductos productsTable={productsTable} setProductsTable={setProductsTable} setAgregarPedidos={setAgregarPedidos} setAuxOrder={setAuxOrder} />}
