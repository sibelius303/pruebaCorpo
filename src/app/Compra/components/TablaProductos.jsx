...
export default function TablaProductos({ productsTable, setProductsTable }) {
	const {enviarReposicion, dataVendor} = useFront()
	...
	return (
		<div className="px-4 sm:px-6 lg:px-8">
			<div className="-mx-4 mt-8 sm:-mx-0">
				<table className="min-w-full divide-y divide-gray-300 bg-BlancoIvory border rounded-md">
					<thead className="bg-azul-pantone text-BlancoIvory">
						<tr className="grid grid-cols-1 gap-4  md:grid-cols-6 ">
					...
					<tbody className="divide-y divide-gray-200 bg-white">
						{productsTable?.map((compra, id) => (
							<tr key={id} className="grid  grid-cols-1 gap-8 md:grid-cols-6">
								...
								<td className=" whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500 ">
									...
									{ dataVendor.find( vendor => vendor.id===compra.idVendor)?.name }
								</td>
			...
			<div className="w-full flex justify-end my-10">
				<button onClick={() => { enviarReposicion(productsTable) }} className="bg-rojo-pantone px-4 py-2.5 rounded-md text-sm font-semibold text-BlancoIvory shadow-sm hover:bg-opacity-50 ">
					Enviar
				</button>
			</div>
		</div>
