...
export default function TablaClientes({setAgregarClientes, setEditState}) {
	...
	const { dataClient, ClientActiveSubmit, reportSubmit} = useFront()
	...
	let arraySlice = dataClient && dataClient?.slice(((pagActual * cantPag) - cantPag), cantPag * pagActual);
    const verificarElementos = Math.ceil(dataClient?.length / cantPag);
	...
	const handlerSupport = async (searchTarget = "") => {
        const url = await reportSubmit(REPORT.CLIENT, {
            id: 0,
            code: "",
            search: searchTarget,
            limit: 0
	...
	return (<>
				...
				<div className="min-w-full divide-y divide-gray-300 border rounded-t-md rounded-b-md">
					<div className="bg-azul-pantone text-BlancoIvory ">
						...
						<div className="grid grid-cols-2 gap-4 w-full md:grid-cols-6 ">
							<div
								scope="col"
								className=" px-3 py-3.5 text-sm text-center font-semibold  sm:table-cell"
							>
								Nombre
							</div>
							<div
								scope="col"
								className=" px-3 py-3.5 text-center text-sm font-semibold  lg:table-cell"
							>
								Identificacion
							</div>
							<div
								scope="col"
								className="hidden md:block px-3 py-3.5 text-center text-sm font-semibold  lg:table-cell"
							>
								Descripcion
							</div>
							<div
					...
					<div className="divide-y divide-gray-200 bg-white">
						{arraySlice.length > 0 ? arraySlice.reverse().map((cliente) => (
							<div key={cliente.id} className="grid grid-cols-2 gap-4 md:grid-cols-6 ">

								{/* <td className=" whitespace-nowrap px-3 py-4 text-sm text-gray-500 sm:table-cell">
									{cliente.id}
								</td> */}
								<div className=" whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500 lg:table-cell">
									{cliente.name}
								</div>
								<div className=" whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500 lg:table-cell">
									{cliente.identification}
								</div>
								<div className=" hidden md:block px-3 py-4 text-sm text-gray-500 text-center lg:table-cell">
									{cliente.descrip}
								</div>
								<div className=" hidden md:block whitespace-break-spaces px-3 py-4 text-center text-sm text-gray-500 lg:table-cell">
									{cliente.address}
								</div>
								<div className=" hidden md:block whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500 lg:table-cell">
									<Toggle defaultActive={cliente?.active} onChange={()=>ClientActiveSubmit(cliente)}/>
								</div>
								<div className=" hidden md:flex whitespace-nowrap  py-4 text-sm w-full px-4  justify-center text-gray-500">
									<button onClick={() => { setAgregarClientes(true), setEditState(prev=>({...prev, editClient: cliente}))}} className=" bg-rojo-pantone w-auto flex justify-center items-center px-4 py-2.5 rounded-md text-sm font-semibold text-BlancoIvory shadow-sm hover:bg-opacity-50 " >
					...
					<div className="px-5 py-5 bg-white flex flex-col xs:flex-row items-center xs:justify-between">
                        ...
                        {/* <span className="text-xs xs:text-sm ">
                            {dataClient?.length === 0 ? (`Mostrando ${dataClient?.length} de ${dataClient?.length} `) : (`Mostrando ${cantPag > dataClient?.length ? dataClient?.length : cantPag } de ${dataClient?.length}`)}
		...
		{ open && <div className="absolute flex flex-col items-center justify-center w-screen h-screen top-0 left-0 bg-slate-500 bg-opacity-50">
            <div className="bg-white w-1/2 h-80 border border-slate-300 flex flex-col gap-4 items-center justify-center rounded-lg shadow-md shadow-slate-600">
                <h2 className="text-xl font-semibold">Generar Reporte Clientes</h2>
                <p className="w-6/12 text-slate-600 text-xs">
                    Use el input para generar un reporte de un Cliente especifico o en su defecto hago click en el boton {'"Reporte Completo"'} para uno mas completo
                </p>
