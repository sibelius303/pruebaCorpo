...
export default function TablaProveedores({setAgregarProveedor, setEditState}) {
	let userString = ""
	...
	const { dataVendor, VendorActiveSubmit, reportSubmit } = useFront()
	...
	let arraySlice = dataVendor && dataVendor?.slice(((pagActual * cantPag) - cantPag), cantPag * pagActual);
    const verificarElementos = Math.ceil(dataVendor?.length / cantPag);
    ...
    const handlerSupport = async (searchTarget = "") => {
        setError( false )
        const url = await reportSubmit(REPORT.VENDOR, {
            id: 0,
            code: "",
            search: searchTarget,
            limit: 0
	...
	return <>
			...
			<div className="-mx-4 mt-8 sm:-mx-0">
				<div className="min-w-full divide-y divide-gray-300 border rounded-t-md rounded-b-md">
					<div className="bg-azul-pantone text-BlancoIvory ">
						...
						<div className="grid grid-cols-2 gap-4 md:grid-cols-5 h-14 ">
							<div
								scope="col"
					...
					<div className="divide-y divide-gray-200 bg-white">
						{arraySlice?.length > 0 ? arraySlice?.reverse().map((pro) => (
							<div key={pro.id} className="grid grid-cols-2 gap-4 md:grid-cols-5 ">
								{/* <td className=" whitespace-nowrap px-3 py-4 text-sm text-gray-500 sm:table-cell">
									{cliente.id}
								</td> */}
								<div className=" hidden md:block whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500 ">
									{pro.code}
								</div>
								...
								<div className=" whitespace-nowrap px-3 py-4 text-sm text-center text-gray-500 ">
									<Toggle defaultActive={pro?.active} onChange={()=>VendorActiveSubmit(pro)}/>
								</div>
								<div className=" hidden md:flex whitespace-nowrap px-4 py-4 text-sm w-full  justify-center text-gray-500 ">
									<button onClick={() => { setAgregarProveedor(true); setEditState(prev=>({...prev, editVendor: pro})) }} className=" bg-rojo-pantone w-auto flex justify-center items-center px-4 py-2.5 rounded-md text-sm font-semibold text-BlancoIvory shadow-sm hover:bg-opacity-50 " >
        ...
        { open && <div className="absolute flex flex-col items-center justify-center w-screen h-screen top-0 left-0 bg-slate-500 bg-opacity-50">
            <div className="bg-white w-1/2 h-80 border border-slate-300 flex flex-col gap-4 items-center justify-center rounded-lg shadow-md shadow-slate-600">
                <h2 className="text-xl font-semibold">Generar Reporte Proveedores</h2>
                <p className="w-6/12 text-slate-600 text-xs">
                    Use el input para generar un reporte de un Proveedor especifico o en su defecto hago click en el boton {'"Reporte Completo"'} para uno mas completo
                </p>
